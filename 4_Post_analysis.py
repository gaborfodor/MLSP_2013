# -*- coding: utf-8 -*-
"""
Created on Thu Aug 22 10:36:33 2013
...
for c in range(CV_FOLDS):
    ...
    for bird in range(num_species):
        ...
        rfr = RandomForestRegressor(n_estimators = N_ESTIMATORS, max_features = MAX_FEATURES, min_samples_split = MIN_SAMPLES_SPLIT,random_state = rs*100, verbose = 0,compute_importances = True)
        ...
        imp_df = imp_df.sort('Importance',ascending = False)
...
for bird in list(spec_avg[spec_avg>0.05].index):
    ...
    bird_image = sp.ndimage.imread(bird_folder+str(bird)+'.jpg')
    imp_df = important_features[bird]
    plt.subplot(NR,NC,1)
    plt.imshow(bird_image)
    plt.axis('off')
    plt.suptitle('Most important features ('+species['species'].irow(bird) +')')
    j=1
    k=0
    #for j in range(1,NC*NR):
    while(j < NC*NR and  k <len(imp_df) ):
        #s = imp_df.Attribute.iloc[j]
        s = imp_df.Attribute.irow(k)
        k=k+1
        if(s[:8] == 'tr_spec_'):
            segment_id = int(s[8:])
    ...
    fig.savefig(bird_folder+'important_features_'+str(bird),facecolor='#fce8ae',edgecolor='none',dpi =300)
